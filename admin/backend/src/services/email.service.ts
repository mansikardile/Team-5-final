...
export class EmailService {
  ...
  public static async sendEventFeedbackInvitation(
    ...
    const feedbackUrl = `http://localhost:3001/feedback?activityCode=${eventDetails.code}`;
      ...
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 24px; overflow: hidden;">
          ...
          <div style="padding: 32px; background-color: #ffffff;">
            ...
            <!-- INLINE SCANNABLE QR CODE IMAGE FOR GMAIL -->
            <div style="background-color: #f8fafc; border: 2px dashed #cbd5e1; border-radius: 20px; padding: 24px; text-align: center; margin: 24px 0;">
              <p style="margin: 0 0 12px 0; font-size: 13px; font-weight: 800; color: #0f2b5c;">
                📱 Scan QR Code with Phone Camera to Submit:
              </p>
              <img src="${qrCodeImageUrl}" alt="Scan QR Code" style="width: 190px; height: 190px; border: 4px solid #0f2b5c; border-radius: 16px; padding: 8px; background-color: #ffffff; shadow: 0 10px 15px -3px rgba(0,0,0,0.1);" />
    ...
    console.log(`[EmailService] 📧 Sending QR Code Email to: ${recipientEmail} (${eventDetails.code})`);
